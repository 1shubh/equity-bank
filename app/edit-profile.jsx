...
const EditProfile = () => {
  const { loading, user, logout, isLogged } = useGlobalContext();
  ...
  const fetchUserByMobile = async () => {
    ...
    try {
      const response = await fetch(
        `https://bank-backend-a00q.onrender.com/api/users/getUserByMobile/${user.user.mobileNumber}`
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `https://bank-backend-1-4cqz.onrender.com/api/users/updateUser/${userData?._id}`, // Update the user endpoint
        {
          method: "PUT",
          headers: {
